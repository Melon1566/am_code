...
describe("RPC authorization scopes", () => {
  it("requires operate access to transfer provider credentials in either direction", () => {
    expect(requiredScopeForRpcMethod(WS_METHODS.providerExport)).toBe(
      AuthOrchestrationOperateScope,
    );
    expect(requiredScopeForRpcMethod(WS_METHODS.providerImport)).toBe(
      AuthOrchestrationOperateScope,
    );
  });
